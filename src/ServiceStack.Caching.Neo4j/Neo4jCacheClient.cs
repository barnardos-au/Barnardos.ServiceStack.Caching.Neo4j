...
namespace ServiceStack.Caching.Neo4j
{
        ...
        where TCacheEntry : ICacheEntry, new()
    {
        ...
        public IDictionary<string, T> GetAll<T>(IEnumerable<string> keys)
        {
            ...
            foreach (var entry in verifiedCacheEntries)
            {
                map[entry.Key] = entry.Value.Deserialize<T>();
            }
        ...
        {
            AutoMapping.RegisterConverter<ZonedDateTime, DateTime>(zonedDateTime => zonedDateTime?.ToDateTimeOffset().DateTime ?? default);
